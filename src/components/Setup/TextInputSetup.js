...
const TextInputSetup = ({ setWarnings, width, done }) => {
    ...
    useEffect(() => {
        AsyncStorage.setItem('warnings', JSON.stringify(warnings));
    }, [warnings])
    ...
    const addWarningSign = () => {
        ...
        const currentWarnings = JSON.parse(JSON.stringify(warnings));
        setWarnings([...currentWarnings, warning]);
    }

    const handleChange = (text, warningId) => {
        let newWarnings = JSON.parse(JSON.stringify(warnings));
